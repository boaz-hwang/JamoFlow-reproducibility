...
import scripts.scale_schedule_extrapolation_core as core
from scripts.run_scale_schedule_extrapolation import _normalized_error
...
def _report(target: int, session: str, *, valid: bool = True) -> dict[str, object]:
    ...
    return {
        ...
        "parameter_count": core.EXPECTED_PARAMETERS[target],
...
def test_schedule_and_model_contracts_are_exact() -> None:
    assert core.TARGET_ORDER == (200, 400, 800, 1600)
    ...
    } == core.EXPECTED_PARAMETERS
    assert core.EXPECTED_PARAMETERS == {
        200: 188_639_808,
        400: 378_058_176,
        800: 790_449_408,
        1600: 1_617_558_528,
    ...
    expected_geometry = {
        ...
        400: (576, 1152, 18, 20),
        800: (768, 1536, 24, 24),
        1600: (1024, 2048, 32, 28),
    }
    for target, (local, global_, heads, layers) in expected_geometry.items():
        spec = core.large_scale_model_spec(target, 86)
        assert (spec.local_width, spec.global_width) == (local, global_)
        assert (spec.local_heads, spec.global_heads) == (heads, heads)
        assert spec.global_layers == layers
        assert local // heads == 32
        assert global_ // heads == 64
...
def test_primary_pass_requires_all_fixed_clauses() -> None:
    summary = core.summarize_scale_schedule_extrapolation(
        timings_by_target=_timings(),
        reports_by_target=_reports(),
    )
    primary = summary["rows"][str(core.PRIMARY_TARGET)]
    assert summary["status"] == "large_scale_10_percent_headroom_detected"
    assert summary["ten_percent_large_scale_headroom_detected"] is True
    assert summary["larger_scale_training_directly_authorized"] is False
    ...
    assert primary["affected_time_share_proxy"] > primary["median_reduction"]
    assert summary["descriptive_scaling"]["scaling_law_fitted"] is False
    assert summary["descriptive_scaling"]["out_of_range_extrapolation_claimed"] is False


def test_smaller_target_cannot_rescue_a_failed_1600m_primary() -> None:
    timings = _timings(candidate=85.0)
    timings[core.PRIMARY_TARGET][..., core.CANDIDATE_INDEX] = 91.0
    summary = core.summarize_scale_schedule_extrapolation(
        timings_by_target=timings,
        reports_by_target=_reports(),
    )
    assert summary["rows"]["800"]["overall_threshold_pass"] is True
    assert summary["rows"][str(core.PRIMARY_TARGET)]["overall_threshold_pass"] is False
    assert summary["ten_percent_large_scale_headroom_detected"] is False
    assert summary["larger_scale_training_directly_authorized"] is False
    assert summary["status"] == "large_scale_10_percent_headroom_not_detected"


def test_point_and_bootstrap_and_prompt_and_session_gates_can_fail() -> None:
    point = core.summarize_scale_schedule_extrapolation(
        timings_by_target=_timings(candidate=91.0),
        reports_by_target=_reports(),
    )
    assert not point["rows"][str(core.PRIMARY_TARGET)]["gates"][
        "point_reduction_at_least_10_percent"
    ]

    with patch.object(core, "_bootstrap_interval", return_value=(0.079, 0.20)):
        bootstrap = core.summarize_scale_schedule_extrapolation(
            timings_by_target=_timings(),
            reports_by_target=_reports(),
        )
    assert not bootstrap["rows"][str(core.PRIMARY_TARGET)]["gates"][
        "bootstrap_lower_at_least_8_percent"
    ]

    prompt_timings = _timings()
    prompt_timings[core.PRIMARY_TARGET][:, :2, :, core.CANDIDATE_INDEX] = 105.0
    with patch.object(core, "_bootstrap_interval", return_value=(0.10, 0.20)):
        prompts = core.summarize_scale_schedule_extrapolation(
            timings_by_target=prompt_timings,
            reports_by_target=_reports(),
        )
    assert not prompts["rows"][str(core.PRIMARY_TARGET)]["gates"][
        "positive_prompts_at_least_15"
    ]

    session_timings = _timings()
    session_timings[core.PRIMARY_TARGET][0, ..., core.CANDIDATE_INDEX] = 105.0
    with patch.object(core, "_bootstrap_interval", return_value=(0.10, 0.20)):
        sessions = core.summarize_scale_schedule_extrapolation(
            timings_by_target=session_timings,
            reports_by_target=_reports(),
        )
    assert not sessions["rows"][str(core.PRIMARY_TARGET)]["gates"][
        "all_three_sessions_positive"
    ]

    stability_timings = _timings(candidate=91.0)
    stability_timings[core.PRIMARY_TARGET][0, ..., core.CANDIDATE_INDEX] = 85.0
    with patch.object(core, "_bootstrap_interval", return_value=(0.10, 0.20)):
        stability = core.summarize_scale_schedule_extrapolation(
            timings_by_target=stability_timings,
            reports_by_target=_reports(),
        )
    assert stability["rows"][str(core.PRIMARY_TARGET)]["gates"][
        "all_three_sessions_positive"
    ]
    assert not stability["rows"][str(core.PRIMARY_TARGET)]["gates"][
...
def test_invalid_evidence_and_timing_arrays_stop_or_raise() -> None:
    ...
    malformed = _timings()
    malformed[core.PRIMARY_TARGET] = malformed[core.PRIMARY_TARGET].astype(np.float32)
    with pytest.raises(ValueError, match="shape/dtype differs"):
        core.summarize_scale_schedule_extrapolation(
            timings_by_target=malformed,
            reports_by_target=_reports(),
        )

    nonfinite = _timings()
    nonfinite[core.PRIMARY_TARGET][0, 0, 0, 0] = np.nan
...
def test_summary_builder_is_canonical_and_tamper_evident() -> None:
    aggregate = core.summarize_scale_schedule_extrapolation(
        timings_by_target=_timings(),
        reports_by_target=_reports(),
    )
    evidence = {
    ...
    tampered["aggregate"]["primary_1600m_pass"] = False
    with pytest.raises(ValueError, match="identity differs"):
        core.validate_scale_schedule_summary(tampered)
    rotated = deepcopy(summary)
    rotated["worker_evidence"][str(core.PRIMARY_TARGET)]["session-2"][
        "timing_path"
    ] = "other.npz"
...
def test_implementation_manifest_is_complete_and_unique() -> None:
    ...
    assert "scripts/verify_scale_schedule_extrapolation.py" in core.IMPLEMENTATION_PATHS
    assert "tests/test_scale_schedule_extrapolation.py" in core.IMPLEMENTATION_PATHS


def test_worker_paths_require_canonical_target_and_session() -> None:
    assert core.worker_report_path(1600, "session-2").name == (
        "target-1600-session-2-report.json"
    )
    with pytest.raises(ValueError, match="identity differs"):
        core.worker_timing_path(101, "session-0")
    with pytest.raises(ValueError, match="identity differs"):
        core.worker_timing_path(1600, "session-9")
