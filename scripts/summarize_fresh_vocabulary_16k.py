#!/usr/bin/env python3
"""Independently replay and summarize the fresh-v2 16K quality screen."""
...
from fresh_vocabulary_16k_core import CANDIDATE_ROLE, ROLES, quality_decision
...
def _context() -> tuple[str, dict[str, Any]]:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("fresh-16k summary requires a clean worktree")
    ...
    if OUTPUT_PATH.exists() or _git(
        ...
        raise RuntimeError("fresh-16k summary cannot be resealed")
    ...
    if (
        ...
        raise RuntimeError("fresh-16k summary context differs")
    ...
    if (
        ...
        or report.get("kind") != "fresh_vocabulary_16k_report_v1"
        ...
        raise RuntimeError("fresh-16k campaign report differs")
    for role in ROLES:
        ...
        if report["workers"][role] != {
            ...
            raise RuntimeError("fresh-16k worker lineage differs")
    return commit, plan
...
def _independent_replay(
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("fresh-16k replay checkpoint differs")
        model = _build_initial_model(role, plan, data)
        model.load_state_dict(state, strict=True)
        if model_parameter_count(model) != worker["parameter_count"]:
            raise RuntimeError("fresh-16k replay parameter count differs")
        ...
        if set(arrays) != set(stored) or any(
            ...
            raise RuntimeError("fresh-16k independent replay differs")
        evidence = {
...
def main() -> None:
    ...
    if any(
        ...
        raise RuntimeError("fresh-16k document denominators differ across roles")
    ...
    baseline = metrics["dense2k_joint_v2"]
    systems = {
        role: {
            "optimizer_step_reduction_vs_dense2k": 1.0
            - metrics[role]["optimizer_steps"] / baseline["optimizer_steps"],
            "training_time_reduction_vs_dense2k": 1.0
            - metrics[role]["optimizer_elapsed_seconds"]
            / baseline["optimizer_elapsed_seconds"],
    ...
    authorized = decision["actual_inference_preflight_authorized"]
    selected_checkpoint = (
        {
            "role": CANDIDATE_ROLE,
            "path": str(
                (CHECKPOINT_ROOT / f"{CANDIDATE_ROLE}.pt").relative_to(ROOT)
            ),
            "artifact_sha256": hash_file(
                CHECKPOINT_ROOT / f"{CANDIDATE_ROLE}.pt"
            ),
            "state_sha256": workers[CANDIDATE_ROLE]["checkpoint_state_sha256"],
        }
        if authorized
        else None
    )
    result: dict[str, Any] = {
        "schema_version": 1,
        "kind": "fresh_vocabulary_16k_quality_one_seed_result_v1",
    ...
    if _git("rev-parse", "HEAD") != commit or _git(
        ...
        raise RuntimeError("repository changed during fresh-16k summary")
    _publish(OUTPUT_PATH, json_bytes(result))
    print(f"status={result['status']}")
    print(f"summary_sha256={result['summary_sha256']}")
    print(f"actual_authorized={authorized}")
