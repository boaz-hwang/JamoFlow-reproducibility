#!/usr/bin/env python3
"""Privacy-audit and promote aggregate-only Phase 3 ecological results."""
...
def _walk(value: Any) -> None:
    if isinstance(value, dict):
        ...
        if value.startswith(("/", "../", "~/")):
            raise ValueError("private path-like string found")
...
def run(args: argparse.Namespace) -> int:
    ...
    if source["privacy"] != required_privacy:
        raise ValueError("privacy guardrail mismatch")
    if not source["patch_integrity"]["all_policies_exactly_86"]:
        raise ValueError("cannot promote a patch-rate invariant failure")
    if source["diagnostic_guard"]["public_gate_replacement"]:
        raise ValueError("private diagnostic cannot replace the public OOD gate")
    _walk(source)
    ...
    print(f"promoted privacy-audited Phase 3 aggregates to {output}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input", default="results/private/phase3-ecological/summary.json"
    )
    parser.add_argument(
        "--output", default="results/phase3-ecological/summary.json"
    )
    return parser
