#!/usr/bin/env python3
"""Run the sealed 50M/75M/100M W72/C86 schedule timing preflight."""
...
def _worker(target: int, session: str) -> None:
    ...
    if (
        count != PUBLICATION_EXPECTED_PARAMETERS[target]
    ...
    report = {
        "schema_version": 1,
        "kind": "scale_schedule_preflight_worker_v1",
...
def _load_worker(
    ...
    *,
    ...
    if (
        ...
        or report["kind"] != "scale_schedule_preflight_worker_v1"
...
def _run_all() -> None:
    ...
    primary = aggregate["rows"]["100"]
    print(f"status={aggregate['status']}")
    print(f"summary_sha256={summary['summary_sha256']}")
    print(f"100m_median_reduction={primary['median_reduction']:.9f}")
    print(f"100m_bootstrap_95_interval={primary['prompt_bootstrap_95_interval']}")
