#!/usr/bin/env python3
"""Run the sealed fresh-v2 2K/8K/16K quality workers."""
...
def _context() -> tuple[str, dict[str, Any]]:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("fresh-16k run requires a clean worktree")
    commit = _git("rev-parse", "HEAD")
    if (
        _git("log", "-1", "--format=%H", "--", str(PLAN_PATH.relative_to(ROOT)))
        != commit
    ):
        raise RuntimeError("fresh-16k plan must be current HEAD")
    ...
    if (
        ...
        or plan["tokenizers"] != selected_tokenizer_identity()
    ):
        raise RuntimeError("fresh-16k run context differs")
    return commit, plan
...
def _load_role_data(role: str, plan: Mapping[str, Any]) -> dict[str, Any]:
    vocabulary_size = int(role_definition(role)["vocabulary_size"])
    ...
    if (
        ...
        raise RuntimeError("fresh-16k token inventory differs")
    ...
    if (
        ...
        raise RuntimeError("fresh-16k raw-byte inventory differs")
    pieces, common = calibration_document_pieces(FRESH_SOURCE_PATH)
    document_inventory, chunks, chunk_documents, document_raw = (
    ...
    if (
        ...
        raise RuntimeError("fresh-16k document inventory differs")
    return {
...
def _build_initial_model(
    ...
    if vocabulary_size == BASE_VOCABULARY_SIZE:
        ...
        if (
            audit.to_dict()
            != initialization["target_initialization_audit_by_vocabulary"][
                str(vocabulary_size)
            ]
        ):
            raise RuntimeError("fresh-16k initialization audit differs")
        expected_state = initialization[
            "target_initial_state_sha256_by_vocabulary"
        ][str(vocabulary_size)]
    if (
        model_parameter_count(model)
        != initialization["parameter_count_by_role"][role]
        or state_mapping_sha256(model.state_dict()) != expected_state
    ):
        raise RuntimeError("fresh-16k initial model differs")
    return model
...
def _all_parameter_optimizer(model: Any) -> torch.optim.Optimizer:
    ...
    if not groups["head"] or sum(map(len, groups.values())) != len(seen):
        raise RuntimeError("fresh-16k optimizer grouping differs")
...
def _new_rows_optimizer(model: Any) -> torch.optim.Optimizer:
    ...
    if len(lexical) != 2:
        raise RuntimeError("fresh-16k staged control requires untied heads")
    ...
    *,
    ...
    if stage_one:
        if (
            ...
            raise RuntimeError("fresh-16k stage-one state differs")
    ...
    if is_geometry_role(role):
        if input_before is None or output_before is None:
            raise AssertionError("fresh-16k update snapshot is missing")
    ...
    if not bool(finite.item()):
        raise RuntimeError("fresh-16k training loss became nonfinite")
    return head_lr
...
def _validate_worker(role: str, commit: str, plan: Mapping[str, Any]) -> bool:
    ...
    if not any(exists):
        return False
    if not all(exists):
        raise RuntimeError(f"partial fresh-16k worker requires forensics: {role}")
    ...
    if (
        ...
        or report.get("kind") != "fresh_vocabulary_16k_worker_v1"
        ...
        raise RuntimeError("completed fresh-16k worker differs")
    ...
    if (
        ...
        raise RuntimeError("completed fresh-16k evidence differs")
    ...
    if (
        ...
        or training.get("seen_sequences")
        != plan["training"][role]["sequence_count"]
        ...
        raise RuntimeError("completed fresh-16k training receipt differs")
    return True
...
def _worker(role: str) -> None:
    ...
    try:
        ...
        with publication_mps_exclusive():
            ...
            optimizer = _new_rows_optimizer(model) if staged else _all_parameter_optimizer(model)
            ...
            for step in range(len(batch_raw)):
                if staged and step == stage_one_steps:
                    stage_one_audit = {
                        ...
                        "new_output_rows_changed": not torch.equal(
                            model.lm_head.weight[BASE_VOCABULARY_SIZE:]
                            .detach()
                            .cpu(),
                            initial_output_new,
                        ),
                    ...
                    if not all(stage_one_audit.values()):
                        raise RuntimeError("fresh-16k stage-one invariant differs")
                    del optimizer
                ...
                last_head_lr = lr
            model.eval()
            evaluation_batch = EVALUATION_BATCH_BY_VOCABULARY[
        ...
        if not timing_environment_eligible(end_state):
            raise RuntimeError("fresh-16k worker environment changed")
        if _git("rev-parse", "HEAD") != commit or _git(
            "status", "--porcelain", "--untracked-files=all"
        ):
            raise RuntimeError("fresh-16k repository changed during training")
        arrays = {
        ...
        report: dict[str, Any] = {
            "schema_version": 1,
            "kind": "fresh_vocabulary_16k_worker_v1",
        ...
        if parameter_count != plan["initialization"]["parameter_count_by_role"][role]:
            raise RuntimeError("fresh-16k trained parameter count differs")
...
def _parent() -> None:
    ...
    if REPORT_PATH.exists() or OUTPUT_PATH.exists():
        raise RuntimeError("fresh-16k downstream output already exists")
    active_payload = json_bytes(
        {
            "kind": "fresh_vocabulary_16k_active_v1",
            "git_commit": commit,
            "plan_artifact_sha256": hash_file(PLAN_PATH),
        }
    )
    if ACTIVE_PATH.exists():
        if ACTIVE_PATH.read_bytes() != active_payload:
            raise RuntimeError("fresh-16k active session differs")
    else:
    ...
    for role in ROLES:
        if not _validate_worker(role, commit, plan):
            raise RuntimeError("fresh-16k worker did not complete")
    ...
    if _git("rev-parse", "HEAD") != commit or _git(
        ...
        raise RuntimeError("fresh-16k repository changed during campaign")
    report: dict[str, Any] = {
        "schema_version": 1,
        "kind": "fresh_vocabulary_16k_report_v1",
        "protocol_id": PROTOCOL_ID,
        "complete": True,
        "git_commit": commit,
        "plan_artifact_sha256": hash_file(PLAN_PATH),
    ...
    _publish(REPORT_PATH, json_bytes(report))
    ACTIVE_PATH.unlink()
    print("status=fresh_vocabulary_16k_workers_complete")
