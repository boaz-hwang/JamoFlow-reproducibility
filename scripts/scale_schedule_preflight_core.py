"""Pure contracts and statistics for the publication-scale schedule preflight."""
...
PROTOCOL_ID = "jamoflow-scale-schedule-preflight-v1"
PLAN_PATH = ROOT / "data/manifests/scale-schedule-preflight-v1.json"
...
ARTIFACT_ROOT = ROOT / "artifacts/scale-schedule-preflight-v1"
ACTIVE_PATH = ARTIFACT_ROOT / ".active"
OUTPUT_PATH = ROOT / "results/scale-schedule-preflight-v1/summary.json"

TARGET_ORDER = tuple(PUBLICATION_SCALE_ORDER)
...
IMPLEMENTATION_PATHS = (
    "docs/185-exaone-retrieval-actual-result-and-core-scale-decision.md",
    "docs/186-scale-schedule-preflight-protocol.md",
    "pyproject.toml",
    "scripts/run_scale_schedule_preflight.py",
    "scripts/scale_schedule_preflight_core.py",
    "scripts/seal_scale_schedule_preflight_plan.py",
    "scripts/verify_scale_schedule_preflight.py",
    ...
    "tests/test_scale_schedule_preflight.py",
)
...
def model_contract() -> dict[str, Any]:
    return {
        str(target): {
            "expected_parameter_count": PUBLICATION_EXPECTED_PARAMETERS[target],
            "spec": publication_model_spec(target, 86).to_dict(),
        }
...
def build_scale_schedule_plan(
    *,
    ...
    payload = {
        "schema_version": 1,
        "kind": "scale_schedule_preflight_plan_v1",
        ...
        "gate": {
            ...
            "stop_rule": "100M primary failure terminates publication-scale W72/C86 training",
        },
        ...
        "threat_model": {
            ...
            "scale_schedule_timing_observed_before_plan": False,
            "timing_used_only_for_100m_training_feasibility": True,
        },
    }
...
def validate_plan(
    ...
    *,
    ...
    if (
        ...
        or value["kind"] != "scale_schedule_preflight_plan_v1"
...
def summarize_scale_schedule_preflight(
    *,
    ...
    for target in TARGET_ORDER:
        ...
        for session, report in zip(SESSION_ORDER, reports, strict=True):
            ...
            parameter_pass = bool(
                ...
                and report["parameter_count"] == PUBLICATION_EXPECTED_PARAMETERS[target]
    ...
    return {
        ...
        "primary_100m_pass": primary_pass,
        "one_seed_100m_training_authorized": primary_pass,
        "status": (
            "one_seed_100m_training_authorized"
            if primary_pass
            else "publication_scale_training_stopped"
        ),
    }
...
def build_scale_schedule_summary(
    *,
    ...
    authorized = aggregate.get("one_seed_100m_training_authorized")
    if not isinstance(authorized, bool):
        raise TypeError("scale-schedule summary authorization differs")
    payload = {
        "schema_version": 1,
        "kind": "scale_schedule_preflight_summary_v1",
        "protocol_id": PROTOCOL_ID,
...
def validate_scale_schedule_summary(value: Mapping[str, Any]) -> None:
    ...
    if (
        value["schema_version"] != 1
        or value["kind"] != "scale_schedule_preflight_summary_v1"
        or value["protocol_id"] != PROTOCOL_ID
        ...
        != {
            ...
            "random_weight_runtime_preflight": True,
            "retrieval_result_changed": False,
            "training_authorized": aggregate.get("one_seed_100m_training_authorized"),
        }
    ):
