#!/usr/bin/env python3
"""Run the sealed seven-role vocabulary-transfer short-CPT probe."""
...
def _context() -> tuple[str, dict[str, Any]]:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("vocabulary-transfer probe requires a clean worktree")
    commit = _git("rev-parse", "HEAD")
    if _git("log", "-1", "--format=%H", "--", str(PLAN_PATH.relative_to(ROOT))) != commit:
        raise RuntimeError("vocabulary-transfer plan must be current HEAD")
    plan = read_json(PLAN_PATH)
    validate_plan(plan)
    if _git("rev-parse", "HEAD^") != plan["git_commit_before_plan"]:
        raise RuntimeError("vocabulary-transfer plan parent differs")
    return commit, plan
...
def _role_data(plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("vocabulary-transfer token inventory differs")
    return {
        "tokenizer": tokenizer,
...
def _optimizer(model: Any) -> torch.optim.Optimizer:
    ...
    if (
        ...
        raise RuntimeError("vocabulary-transfer optimizer grouping differs")
...
def _effective_step(
    model: Any, optimizer: torch.optim.Optimizer, batch: np.ndarray, *, step: int
) -> None:
    for group in optimizer.param_groups:
        group["lr"] = (
            BODY_LEARNING_RATE
            if group["schedule_kind"] == "body"
            else probe_learning_rate(
                step,
                peak=HEAD_PEAK_LEARNING_RATE,
                minimum=HEAD_MINIMUM_LEARNING_RATE,
            )
        )
    ...
    if not bool(finite.item()):
        raise RuntimeError("vocabulary-transfer training loss became nonfinite")
    ...
    with torch.inference_mode():
        for start in range(0, len(sequences), EVALUATION_BATCH_SIZE):
            ...
            if not torch.isfinite(row).all():
                raise RuntimeError("vocabulary-transfer NLL became nonfinite")
...
def _validate_worker(role: str, commit: str, plan: Mapping[str, Any]) -> bool:
    ...
    if not any(existing):
        return False
    if not all(existing):
        raise RuntimeError(f"partial vocabulary-transfer worker requires forensics: {role}")
    ...
    if (
        ...
        or report.get("kind") != "vocabulary_transfer_probe_worker_v1"
        ...
        or not isinstance(
            training.get("elapsed_seconds_including_checkpoint_evaluations"),
            (int, float),
        )
        or not math.isfinite(
            float(training["elapsed_seconds_including_checkpoint_evaluations"])
        )
        ...
        raise RuntimeError("completed vocabulary-transfer worker differs")
    ...
    for step in PROBE_STEPS:
        ...
        arrays_metadata = row.get("arrays") if isinstance(row, Mapping) else None
        if (
            ...
            or not isinstance(arrays_metadata, Mapping)
            or set(arrays_metadata) != {"nll_nats", "raw_target_bytes"}
            ...
            raise RuntimeError("vocabulary-transfer checkpoint lineage differs")
        state = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
        if (
            not isinstance(state, Mapping)
            or any(not isinstance(value, torch.Tensor) for value in state.values())
            or state_mapping_sha256(state) != row.get("checkpoint_state_sha256")
        ):
        ...
        with np.load(nll_path, allow_pickle=False) as archive:
            if set(archive.files) != {"nll_nats", "raw_target_bytes"}:
                raise RuntimeError("vocabulary-transfer NLL array set differs")
            for name in archive.files:
                values = archive[name]
                if arrays_metadata[name] != {
                    "dtype": str(values.dtype),
                    "shape": list(values.shape),
                    "sha256": array_sha256(values),
                }:
                    raise RuntimeError("vocabulary-transfer NLL array differs")
            ...
            if (
                ...
                raise RuntimeError("vocabulary-transfer NLL semantics differ")
    return True


def _worker(role: str) -> None:
    commit, plan = _context()
    if role not in TRANSFER_ROLES or _validate_worker(role, commit, plan):
        return
    ...
    if array_sha256(order[: FINAL_PROBE_STEP * EFFECTIVE_BATCH_SIZE]) != plan["training"][
        ...
        raise RuntimeError("vocabulary-transfer order differs")
    ...
    if int(raw_target_bytes.sum()) != data["calibration_inventory"].predicted_target_raw_bytes:
        raise RuntimeError("vocabulary-transfer denominator differs")
    loaded_tokenizers = load_tokenizers()
    base_tokenizer, base_token_bytes = loaded_tokenizers[2_048]
    target_tokenizer, _ = loaded_tokenizers[TARGET_VOCABULARY_SIZE]
    ...
    if not timing_environment_eligible(start_state):
        raise RuntimeError("vocabulary-transfer worker environment is ineligible")
    ...
    with publication_mps_exclusive():
        model, audit = build_transferred_model(
            ...
            base_pieces=base_token_bytes,
            target_pieces=data["token_bytes"],
            decompositions=decompositions,
        )
        if audit.to_dict() != plan["initialization_audits"][role]:
            raise RuntimeError("vocabulary-transfer initialization audit differs")
        ...
        if state_mapping_sha256(initial_state) != plan["initial_state_sha256"][role]:
            raise RuntimeError("vocabulary-transfer initial state differs")
        model = model.to("mps")
        ...
        for checkpoint_step in PROBE_STEPS:
            while finite_steps < checkpoint_step:
                ...
                model.train()
                _effective_step(model, optimizer, batch, step=finite_steps)
                finite_steps += 1
            model.eval()
    ...
    if not timing_environment_eligible(end_state):
        raise RuntimeError("vocabulary-transfer worker environment changed")
    report_path, checkpoint_paths, nll_paths = _paths(role)
    checkpoints = {}
    for step in PROBE_STEPS:
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "kind": "vocabulary_transfer_probe_worker_v1",
    ...
    # Nothing enters the canonical namespace until every checkpoint, NLL and
    # receipt has been constructed and validated in memory.
    for step in PROBE_STEPS:
...
def _parent() -> None:
    ...
    if REPORT_PATH.exists() or OUTPUT_PATH.exists():
        raise RuntimeError("vocabulary-transfer downstream output already exists")
    active_payload = json_bytes(
        {
            "git_commit": commit,
            "kind": "vocabulary_transfer_probe_active_v1",
            "plan_artifact_sha256": hash_file(PLAN_PATH),
        }
    )
    if ACTIVE_PATH.exists():
        if ACTIVE_PATH.read_bytes() != active_payload:
            raise RuntimeError("vocabulary-transfer active session differs")
    else:
    ...
    for role in TRANSFER_ROLES:
        if not _validate_worker(role, commit, plan):
            raise RuntimeError("vocabulary-transfer worker did not complete")
        path = _paths(role)[0]
        workers[role] = {
    ...
    if _git("rev-parse", "HEAD") != commit or _git(
        ...
        raise RuntimeError("repository changed during vocabulary-transfer probe")
    report = {
        "schema_version": 1,
        "kind": "vocabulary_transfer_probe_report_v1",
        "protocol_id": PROTOCOL_ID,
        "complete": True,
        "git_commit": commit,
        "plan_artifact_sha256": hash_file(PLAN_PATH),
    ...
    _publish(REPORT_PATH, json_bytes(report))
    ACTIVE_PATH.unlink()
    print("status=vocabulary_transfer_probe_workers_complete")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--worker", choices=TRANSFER_ROLES)
