#!/usr/bin/env python3
"""Read-only reconstruction of the sealed scale-schedule extrapolation."""
...
def _independent_correctness_replay(
    ...
    with publication_mps_exclusive(), torch.inference_mode():
        for target in TARGET_ORDER:
            ...
            if (
                parameter_count(model) != EXPECTED_PARAMETERS[target]
                or parameter_count(model) != expected["expected_parameter_count"]
